...
def check_links(topics: dict[str, Topic]) -> None:
    for t in topics.values():
        for slug in LINK.findall(t.body):
...
{body}
  ...
  <footer>
    Gallery Sync — How To Guide. Last updated {LAST_UPDATED}.
  </footer>
...
def build_flat(topics: dict[str, Topic]) -> str:
    ...
    for chapter in CHAPTERS:
        ...
        for t in chapter.topics:
            ...
            parts.append(blocks_html(t.body, topics, "h4"))
...
def popup_body(t: Topic, topics: dict[str, Topic]) -> str:
    ...
    out: list[str] = []
    for kind, value in parse(t.body):
        if kind == "h":
            out.append(f"## {value}")
        elif kind == "p":

...
object Migrations {
    ...
    /**
     * Adds the record of files that left the phone without ever being sent. See [UnsentDepartureEntity].
     *
     * Purely additive: a new table, nothing altered, nothing dropped. Existing installs come through
     * with it empty, which is right, because the files it would describe were forgotten before it
     * existed and cannot be recovered. Ian, 19 Sept 2026: the window that opens with the app covers all
     * deleted files, not only the ones this app backed up.
     */
    val MIGRATION_10_11 = object : Migration(10, 11) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL(
                """
                CREATE TABLE IF NOT EXISTS `unsent_departures` (
                    `id` TEXT NOT NULL,
                    `mediaStoreId` INTEGER NOT NULL,
                    `contentUri` TEXT NOT NULL,
                    `displayName` TEXT NOT NULL,
                    ...
                    `mimeType` TEXT NOT NULL,
                    `isVideo` INTEGER NOT NULL,
                    `goneSinceEpochMillis` INTEGER NOT NULL,
                    PRIMARY KEY(`id`)
                )
                """.trimIndent()
            )
